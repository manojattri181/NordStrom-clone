...
export default function Productgift() {
 ...
 useEffect(() => {
   axios.get('https://json-server-manoj-api.herokuapp.com/posts?q=women').then((res)=>{
       setData(res.data);
    })
}, [])
...
function NextArrow(props) {
  ...
  return (
    ...
    <Slider {...settings} >
        {
          Data.map((items ,i)=>(
            <Box key={i} textAlign="center" p="5px" position={"relative"} >
                 <Badge colorScheme='green' zIndex="10" variant='subtle' position={"absolute"} right="10" top="1" p="3px" fontSize="12px" >{items.new_collection ? "New" : "Limited Stock"}
                 </Badge>
